...
function init()
	...
	message.setHandler("despawn", function()
		...
		self.shouldDie = true
		world.sendEntityMessage(self.childId, "despawn")
...
function move()
	...
	if self.inGround then
		...
		if self.targetId then
				if attacking then
						if world.magnitude(targetPosition, mcontroller.position()) > 20 then
								attacking = false
								animator.setAnimationState("body","close")
						end
				elseif world.magnitude(targetPosition, mcontroller.position()) < 20 then
						attacking = true
