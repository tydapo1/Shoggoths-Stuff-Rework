...
function init()
  ...
  self.searchDistance = config.getParameter("searchDistance", 0.7)
...
function update(dt)
  --If the projectile has collided with tiles dont reduce its time to live
...
function anchored()
	return (mcontroller.stickingDirection() or self.stuckToTarget)
end
...
function removeOwnId(array)
	for i, target in ipairs(array) do
		--sb.logInfo("Own ID = " .. entity.id())
		--sb.logInfo("Target ID = " .. entity.id())
