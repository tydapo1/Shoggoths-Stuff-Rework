...
function update(dt)
	local erchiusRatio = math.sqrt(math.min(1.0, self.effectMaxErchius / self.effectMaxErchius))
	if self.effectMaxErchius > 0 and self.spawnTimer > 0 then
	...
	if monsterPosition then
		...
		world.sendEntityMessage(self.monsterUniqueId, "setErchiusLevel", self.effectMaxErchius)
...
function uninit()
	world.sendEntityMessage(self.monsterUniqueId, "despawn")
end
