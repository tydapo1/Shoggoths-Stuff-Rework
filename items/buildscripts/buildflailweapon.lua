...
function build(directory, config, parameters, level, seed)
	...
	if config.tooltipKind ~= "base" then
		...
		config.tooltipFields.rangeLabel = util.round(config.primaryAbility.maxLength or 0)
		config.tooltipFields.ballspeedLabel = util.round(config.primaryAbility.projectileParameters.speed or 0)
		config.tooltipFields.knockbackLabel = util.round(config.primaryAbility.projectileParameters.knockback or 0)
