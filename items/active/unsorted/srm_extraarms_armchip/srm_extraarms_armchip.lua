...
function init()
	...
	self.thing_to_unlock = config.getParameter("thing_to_unlock")
...
function update(dt, fireMode, shiftHeld)
	...
	if storage.firing and animator.animationState("firing") == "off" then
		if player then
			world.sendEntityMessage(player.id(), "extraarms_setProperty", "extraarms_unlock_" .. self.thing_to_unlock, true)
			world.sendEntityMessage(player.id(), "extraarms_setProperty", "extraarms_enabled_left_" .. self.thing_to_unlock, true)
			world.sendEntityMessage(player.id(), "extraarms_setProperty", "extraarms_enabled_right_" .. self.thing_to_unlock, true)
			item.consume(1)
		end
