...
function Punch:update(dt, fireMode, shiftHeld)
	...
	if self.freezeTimer > 0 and not mcontroller.onGround() and math.abs(world.gravity(mcontroller.position())) > 0 then
		local angle = world.entityPosition(activeItem.ownerEntityId())
		angle = vec2.angle(world.distance(activeItem.ownerAimPosition(),angle))
		angle = vec2.rotate({5,0}, angle)
		mcontroller.controlApproachVelocity(angle, 500)
		--mcontroller.addMomentum(angle)
...
function Punch:fire()
	...
	util.wait(self.stances.fire.duration, function()
		...
		local kback = 10 * mcontroller.facingDirection()
		self.damageConfig.knockback[1] = kback
